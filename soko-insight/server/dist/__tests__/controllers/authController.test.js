...
describe('Auth Controller', () => {
    describe('GET /api/auth/plans', () => {
        it('should return public pricing plans', async () => {
            ...
            const planTiers = response.body.data.plans.map((plan) => plan.tier);
    ...
    describe('POST /api/auth/register', () => {
        ...
        it('should create default sales channels for small_trader', async () => {
            ...
            const channels = await SalesChannel_1.SalesChannelModel.findByUserId(userId);
    ...
    describe('GET /api/auth/me', () => {
        it('should return current user with valid token', async () => {
            ...
            const response = await (0, supertest_1.default)(app)
                .get('/api/auth/me')
                .set((0, testHelpers_1.getAuthHeader)(testUser.token))
                .expect(200);
            expect(response.body.success).toBe(true);
    ...
    describe('PUT /api/auth/change-password', () => {
        it('should change password with correct current password', async () => {
            const testUser = await (0, testHelpers_1.createTestUser)({ password: 'OldPass123' });
            const response = await (0, supertest_1.default)(app)
        ...
        it('should reject if new password is same as current', async () => {
            const testUser = await (0, testHelpers_1.createTestUser)({ password: 'SamePass123' });
            const response = await (0, supertest_1.default)(app)
                .put('/api/auth/change-password')
                .set((0, testHelpers_1.getAuthHeader)(testUser.token))
                .send({
    ...
    describe('POST /api/auth/deactivate', () => {
        it('should deactivate account with correct password', async () => {
            const testUser = await (0, testHelpers_1.createTestUser)({ password: 'DeactivatePass123' });
            const response = await (0, supertest_1.default)(app)
        ...
        it('should reject deactivation with incorrect password', async () => {
            const testUser = await (0, testHelpers_1.createTestUser)({ password: 'CorrectPass123' });
            const response = await (0, supertest_1.default)(app)
                .post('/api/auth/deactivate')
                .set((0, testHelpers_1.getAuthHeader)(testUser.token))
                .send({
