...
exports.getCategoryPerformance = exports.getChannelComparison = exports.getProductPerformance = exports.getTrends = exports.getOverview = void 0;
...
exports.getOverview = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Get summary
    const summary = await SalesData_1.SalesDataModel.getSummary(userId, start, end);
    // Get previous period for comparison
    const periodDays = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
    const prevStart = new Date(start.getTime() - periodDays * 24 * 60 * 60 * 1000);
    const prevEnd = start;
    const prevSummary = await SalesData_1.SalesDataModel.getSummary(userId, prevStart, prevEnd);
    // Calculate trends
    ...
    const profitTrend = prevSummary.totalProfit > 0
        ...
        : 0;
    // Get daily sales
    const dailySales = await SalesData_1.SalesDataModel.getDailySales(userId, start, end);
    // Get top products
    const topProducts = await SalesData_1.SalesDataModel.getTopProducts(userId, 10, start, end);
    // Get sales by channel
    const salesByChannel = await SalesData_1.SalesDataModel.getSalesByChannel(userId, start, end);
...
exports.getTrends = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    ORDER BY period ASC
  `;
    const { rows } = await (0, database_1.query)(sql, [userId, start, end]);
    ...
    return (0, response_1.sendSuccess)(res, { trends, granularity });
});
...
exports.getProductPerformance = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const end = endDate ? new Date(endDate) : new Date();
    // Get top products
    const topProducts = await SalesData_1.SalesDataModel.getTopProducts(userId, parseInt(limit, 10), start, end);
    // Get slow movers (products with no sales in the period)
    const { rows: slowMoversRows } = await (0, database_1.query)(`SELECT p.id, p.name, p.current_stock, p.selling_price, p.reorder_level
     FROM products p
     WHERE p.user_id = $1 
    ...
    const { rows: productTrendsRows } = await (0, database_1.query)(`SELECT 
      p.id as product_id,
    ...
    ORDER BY sd.sale_date ASC, revenue DESC`, [userId, start, end]);
...
exports.getChannelComparison = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { startDate, endDate } = req.query;
    const userId = req.user.id;
    const start = startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const end = endDate ? new Date(endDate) : new Date();
    ...
    const { rows: channelTrendsRows } = await (0, database_1.query)(`SELECT 
      sc.id as channel_id,
      sc.channel_name,
      sc.channel_type,
      TO_CHAR(sd.sale_date, 'YYYY-MM-DD') as date,
    ...
    const bestChannel = channelMetrics.length > 0
        ? channelMetrics.reduce((best, current) => current.revenue > best.revenue ? current : best)
        : null;
    const worstChannel = channelMetrics.length > 0
        ? channelMetrics.reduce((worst, current) => current.revenue < worst.revenue ? current : worst)
        : null;
    return (0, response_1.sendSuccess)(res, {
...
exports.getCategoryPerformance = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { startDate, endDate } = req.query;
    const userId = req.user.id;
    const start = startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const end = endDate ? new Date(endDate) : new Date();
    ...
    ORDER BY total_revenue DESC`, [userId, start, end]);
