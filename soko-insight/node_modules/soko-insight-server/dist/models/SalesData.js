...
exports.SalesDataModel = void 0;
const database_1 = require("../config/database");
function rowToSalesData(row) {
    return {
        id: row.id,
        userId: row.user_id,
...
function rowToSalesDataWithDetails(row) {
    return {
        ...rowToSalesData(row),
...
class SalesDataModel {
    ...
    static async findByUserId(userId, options = {}) {
        const { page = 1, limit = 50, startDate, endDate, productId, channelId, sortBy = 'sale_date', sortOrder = 'desc', } = options;
        const offset = (page - 1) * limit;
        const conditions = ['sd.user_id = $1'];
        const params = [userId];
        let paramIndex = 2;
        if (startDate) {
        ...
        // Get total count
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM sales_data sd WHERE ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].total, 10);
        // Get sales with product and channel names
        params.push(limit, offset);
        const { rows } = await (0, database_1.query)(`SELECT sd.*, p.name as product_name, sc.channel_name
       FROM sales_data sd
    ...
    static async create(input) {
        ...
        const { rows } = await (0, database_1.query)(`INSERT INTO sales_data (
    ...
    static async delete(id, userId) {
        const { rowCount } = await (0, database_1.query)('DELETE FROM sales_data WHERE id = $1 AND user_id = $2', [id, userId]);
        return rowCount > 0;
    }
    ...
    static async getDailySales(userId, startDate, endDate) {
       ...
       GROUP BY sale_date
       ORDER BY sale_date ASC`, [userId, startDate, endDate]);
        return rows.map(row => ({
            date: row.date,
    ...
    static async getTopProducts(userId, limit = 10, startDate, endDate) {
        let sql = `
      SELECT 
        p.id as product_id,
    ...
    static async getSalesByChannel(userId, startDate, endDate) {
        let sql = `
      SELECT 
        sc.id as channel_id,
        sc.channel_name,
...
exports.SalesDataModel = SalesDataModel;
