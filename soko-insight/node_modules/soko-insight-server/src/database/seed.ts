...
import { getDefaultSubscriptionTier, SellerType } from '../config/pricing';
...
async function seedDatabase(): Promise<void> {
  ...
  try {
    ...
    const categoryResult = await query('SELECT id, name FROM categories WHERE is_system = true');
    ...
    for (const userData of testUsers) {
      ...
      if (existingUser.rows.length > 0) {
        ...
        const userResult = await query(
          `INSERT INTO users (id, email, password_hash, business_name, phone, seller_type, subscription_tier, is_verified)
          ...
          [
            ...
            userData.phone,
            userData.sellerType,
            getDefaultSubscriptionTier(userData.sellerType),
            true,
          ]
        );
      ...
      if (userData.sellerType === 'small_trader') {
        ...
        for (const channel of channels) {
          const channelResult = await query(
            `INSERT INTO sales_channels (id, user_id, channel_name, channel_type, platform)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT DO NOTHING
             RETURNING id`,
            [uuidv4(), userId, channel.name, channel.type, channel.platform]
          );
      ...
      if (channelIds.length === 0) {
        const existingChannels = await query(
          'SELECT id FROM sales_channels WHERE user_id = $1',
          [userId]
        );
        channelIds.push(...existingChannels.rows.map((r: any) => r.id));
      }
      ...
      for (const product of products) {
        ...
        const productResult = await query(
          `INSERT INTO products (id, user_id, category_id, name, sku, cost_price, selling_price, current_stock, reorder_level)
      ...
      if (productIds.length === 0) {
        const existingProducts = await query(
          'SELECT id FROM products WHERE user_id = $1 LIMIT 8',
          [userId]
        );
        productIds.push(...existingProducts.rows.map((r: any) => r.id));
      }
      ...
      for (let i = 0; i < 90; i++) {
        ...
        for (let j = 0; j < salesCount; j++) {
          ...
          // Get product details
          const productResult = await query(
            'SELECT selling_price, cost_price FROM products WHERE id = $1',
            [productId]
          );
      ...
      if (salesData.length > 0) {
        ...
        for (let i = 0; i < salesData.length; i += batchSize) {
          ...
          for (const sale of batch) {
            try {
              await query(
                `INSERT INTO sales_data (id, user_id, product_id, channel_id, sale_date, quantity, unit_price, total_amount, cost_amount, profit_amount, entry_method)
      ...
      if (userData.sellerType === 'ecommerce') {
        ...
        for (const competitor of competitors) {
          const competitorResult = await query(
            `INSERT INTO competitors (id, user_id, name, platform, website)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT DO NOTHING
             RETURNING id`,
            [uuidv4(), userId, competitor.name, competitor.platform, competitor.website]
          );
        ...
        if (competitorIds.length > 0 && productIds.length > 0) {
          ...
          for (let i = 0; i < pricesToAdd; i++) {
            ...
            if (productResult.rows.length > 0) {
              ...
              await query(
                `INSERT INTO competitor_prices (id, product_id, competitor_id, price)
                 VALUES ($1, $2, $3, $4)
                 ON CONFLICT DO NOTHING`,
                [uuidv4(), productId, competitorId, competitorPrice]
              );
            }
          }
